...
from app.legacy.services import captures, exports, migrations, segments
...
async def get_list(query: Annotated[GetAudioCaptureListRequest, Query()], db: DBSession) -> BaseResponse:
    items, total = await captures.get_audio_capture_list(db=db, query=query)
    return BaseResponse(
...
async def get_by_id(audio_capture_id: UUID, db: DBSession, storage: Storage) -> BaseResponse:
    return BaseResponse(
        ...
        data=await captures.get_audio_capture_detail(audio_capture_id=audio_capture_id, db=db, storage=storage),
    )
...
async def create_audio_segment(
    ...
    return BaseResponse(
        ...
        data=await segments.create_audio_segment(audio_capture_id=audio_capture_id, data=body, db=db, storage=storage),
    )
...
async def trim_audio_segment(
    ...
    return BaseResponse(
        ...
        data=await segments.trim_audio_segment(audio_segment_id=audio_segment_id, data=body, db=db, storage=storage),
    )
...
async def assign_audio_segment_label(
    ...
    return BaseResponse(
        ...
        data=await segments.assign_audio_segment_label(audio_segment_id=audio_segment_id, data=body, db=db),
    )
...
async def update_audio_segment_memo(
    ...
    return BaseResponse(
        ...
        data=await segments.update_audio_segment_memo(audio_segment_id=audio_segment_id, data=body, db=db),
    )
...
async def delete_audio_segment(audio_segment_id: UUID, db: DBSession) -> BaseResponse:
    return BaseResponse(
        ...
        data=await segments.delete_audio_segment(audio_segment_id=audio_segment_id, db=db),
    )
...
async def assign_audio_capture_labels(
    ...
    return BaseResponse(
        ...
        data=await captures.assign_audio_capture_labels(audio_capture_id=audio_capture_id, data=body, db=db),
    )
...
async def update_audio_capture_memo(
    ...
    return BaseResponse(
        ...
        data=await captures.update_audio_capture_memo(audio_capture_id=audio_capture_id, data=body, db=db),
    )
...
async def migrate_reviews(body: MigrateReviewsRequest, db: DBSession) -> BaseResponse:
    return BaseResponse(message="리뷰 마이그레이션 성공", data=await migrations.migrate_reviews(data=body, db=db))
...
async def export_audio_segments(
    ...
    return Response(
        content=await exports.export_audio_segments(
