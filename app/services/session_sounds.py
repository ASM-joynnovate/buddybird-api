...
from app.models import Device, File, Session, SessionSound, SoundJudgment
...
async def get_list(
    *, db: AsyncSession, storage: S3StorageClient, session: Session, query: PageParams
    ...
    stmt = select(SessionSound).where(SessionSound.session_id == session.id, SessionSound.is_parrot_sound.is_(True))
