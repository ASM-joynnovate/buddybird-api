...
class CustomKeyMaker(BaseKeyMaker):
    async def make(self, *, function: Callable, prefix: str, bound_args: inspect.BoundArguments) -> str:
        ...
        for idx, arg in enumerate(inspect.signature(function).parameters.values()):
            if (
                    arg.name in ("self", "cls") or
                    arg.kind in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD)
            ):
                continue
            ...
            if arg.name in bound_args.arguments:
                ...
                elif isinstance(bound_args.arguments[arg.name], list):
                    args += f"={sorted(bound_args.arguments[arg.name])}"
                elif isinstance(bound_args.arguments[arg.name], set):
                    args += f"={sorted(bound_args.arguments[arg.name])}"
                elif isinstance(bound_args.arguments[arg.name], tuple):
                    args += f"={sorted(bound_args.arguments[arg.name])}"
